...
class DQNAgent:
    ...
    def train(self, terminal_state, step):
        ...
        for i in norm_qs:
            print(i.shape)
            i.reshape(env.ACTION_SPACE_SIZE ** env.NUM_OF_PLAYERS,)
            future_qs_list.append(self.target_model_quanta["test"](i,self.weights)) 
        ...
        corr = np.array(y)
        self.weights = self.model_quanta["train"](inp, corr, 2)
...
for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
    ...
    if step >= AGGREGATE_STATS_EVERY and not episode % AGGREGATE_STATS_EVERY or episode == 1:
        ...
        f.write(str(step) + " " + str(average_reward) + " " + str(min_reward) + " " + str(max_reward))
