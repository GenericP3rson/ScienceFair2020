...
f = open(str(time.time()) + "_Model", "a")

f.write("STEP AVERAGE_REWARD MIN_REWARD MAX_REWARD")
...
class DQNAgent:
    def __init__(self):
        ...
        self.model_quanta = qnn.qnn()
        self.model_quanta["init"](2)
        self.weights = self.model_quanta["get_var"]()

        # Target network
        self.target_model = self.create_model()
        self.target_model.set_weights(self.model.get_weights())
        self.target_model_quanta = qnn.qnn()
        self.target_model_quanta["init"](2)
        self.target_model_quanta["set_var"](self.model_quanta["get_var"]())
    ...
    def create_model(self):
        ...
        model.add(Dense(env.ACTION_SPACE_SIZE **
                        env.NUM_OF_PLAYERS, activation='linear'))
        model.summary()
    ...
    def train(self, terminal_state, step):
        ...
        current_qs_list = []
        normal_current_qs = self.model.predict(current_states).reshape(
            MINIBATCH_SIZE, env.ACTION_SPACE_SIZE**env.NUM_OF_PLAYERS,)
        for i in normal_current_qs:
            # print(i.shape)
            i.reshape(env.ACTION_SPACE_SIZE ** env.NUM_OF_PLAYERS,)
            current_qs_list.append(self.model_quanta["test"](i, self.weights))

        # Get future states from minibatch, then query NN model for Q values
        # When using target network, query it, otherwise main network should be queried
        new_current_states = np.array(
            [transition[3] for transition in minibatch])/255  # New Image
        norm_qs = self.target_model.predict(new_current_states).reshape(
            MINIBATCH_SIZE, env.ACTION_SPACE_SIZE ** env.NUM_OF_PLAYERS,)
        future_qs_list = []
        for i in norm_qs:
            # print(i.shape)
            i.reshape(env.ACTION_SPACE_SIZE ** env.NUM_OF_PLAYERS,)
            future_qs_list.append(
                self.target_model_quanta["test"](i, self.weights))
        ...
        self.model.fit(np.array(X)/255, np.array(y), batch_size=MINIBATCH_SIZE, verbose=0,
                       shuffle=False)
        inp = self.model.predict(np.array(X)/255)
        corr = np.array(y)
        # print(inp.shape, corr.shape)
        for i in range(len(inp)):
            param1 = inp[i].reshape(inp.shape[1],)
            param2 = corr[i].reshape(corr.shape[1], )
            # print(param1, param2)
            # print(param1.shape, param2.shape)
            self.weights = self.model_quanta["train"](param1, param2, 1)
        ...
        if self.target_update_counter > UPDATE_TARGET_EVERY:
            self.target_model.set_weights(self.model.get_weights())
            self.target_model["set_var"](self.model_quanta["get_var"]())
            self.target_update_counter = 0

    # Queries main network for Q values given current observation space (environment state)
    def get_qs(self, state):
        return self.model_quanta["test"](self.model.predict(np.array(state).reshape(-1, *state.shape)/255).reshape(env.ACTION_SPACE_SIZE**env.NUM_OF_PLAYERS,),
                                         self.weights)
...
for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
    ...
    while not done:
        ...
        if np.random.random() > epsilon:
            ...
            # Getting the max value of action combinations
            unique_action_index = np.argmax(current_qs)
            ...
            current_qs = agent.get_qs(current_state)
            unique_action_index = np.random.randint(
                0, env.ACTION_SPACE_SIZE**env.NUM_OF_PLAYERS)
            actions = convert_num_to_action_array(unique_action_index)
            # for i in range(env.ACTION_SPACE_SIZE, len(current_qs)+1, env.ACTION_SPACE_SIZE):
    ...
    if step >= AGGREGATE_STATS_EVERY and not episode % AGGREGATE_STATS_EVERY or episode == 1:
        ...
        f.write("\n" +str(step) + " " + str(average_reward) +
                " " + str(min_reward) + " " + str(max_reward))
        print(("STEP: " + str(step) + "\nAVG_REWARD: " + str(average_reward) +
