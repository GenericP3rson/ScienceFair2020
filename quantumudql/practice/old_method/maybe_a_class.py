...
class CV_QNN():
    def __init__(self, number_of_players):
        ...
        self.cutoff_dim = max([25, 17**number_of_players])  # Max number of nodes per layer

        self.run([i/25 for i in range(25)])

        # self.weights = np.zeros((self.layers, ))
    ...
    def parse_inputs(self, inputs):
        # Temporary fix. Like a ripoff CNN.
        new_inputs = []
        for i in range(0, len(inputs), 25):
            new_inputs.append(sum(inputs[i:i+25])/25)
        return new_inputs
    ...
    def run(self, fresh_inputs, desired_outputs = []):
        ...
        self.target_state = tf.constant(
            target_state, dtype=tf.complex64)  # Desired output
        # self.target_state = desired_outputs

        self.modified_inputs = tf.placeholder(tf.float32, shape=[self.cutoff_dim])
