...
cutoff_dim = 6  # Max number of nodes per layer

# defining desired state (single photon state)
target_state = np.zeros(cutoff_dim)
target_state[1] = 1
target_state = tf.constant(target_state, dtype=tf.complex64)  # Desired output
...
modified_inputs = tf.placeholder(tf.float32, shape=[layers])
...
print("\nTarget state: " + str(sess.run(target_state, feed_dict=feed_dict)))
