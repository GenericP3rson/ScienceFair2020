...
EPISODES = 25 # Number of interations
...
class DQNAgent:
    ...
    def train(self, terminal_state, step):
        ...
        for index, (current_state, action, reward, new_current_state, done) in enumerate(minibatch):
            ...
            # TODO: Change max to averafe of maxes?
            if not done:
...
for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
    ...
    while not done:
        ...
        if np.random.random() > epsilon:
            ...
            action = np.argmax(agent.get_qs(current_state)[:env.ACTION_SPACE_SIZE])
        ...
        new_state, reward, done = env.step([action, action2])
