...
start_q_table = "qtable-1580298187.pickle"  # None or Filename
...
class Blob:
    def __init__(self):
        self.x = np.random.randint(0, SIZE) # Starts at a random position
...
if start_q_table is None:
    ...
    q_table = {}
    for i in range(-SIZE+1, SIZE): # x
        for ii in range(-SIZE+1, SIZE): # y
            for iii in range(-SIZE+1, SIZE): # fire
                    for iiii in range(-SIZE+1, SIZE): # food
...
# 'Tis the difference between the (player and fire) and the (player and enemy)
